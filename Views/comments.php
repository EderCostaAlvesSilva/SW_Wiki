...
<body>
    ...
    <script>
        ...
        function deleteComments(user, route) {
        ...
        async function renderComments(data) {
            ...
            if (data.results && Array.isArray(data.results)) {
                ...
                data.results.forEach(item => {
                    ...
                    containerDiv.classList.add("m-2", "col-12", "col-sm-5", "col-md-3", "border", "rounded", "d-flex", "flex-column", "align-items-center", "justify-content-center", "text-center", "p-2");

                    const link = document.createElement("a");
                    link.classList.add("text-warning");
                    link.href = '/L5_Networks' + item.route;

                    const [datePart, timePart] = item.datahora.split(' ');
                    const [year, month, day] = datePart.split('-');
                    const [hour, minute] = timePart.split(':');

                    link.innerHTML = `
                        <h3>Topic: ${item.titulo}</h3>
                        <p>Comment: ${item.comentario}</p>
                    ...
                    if (nivel == 'A') {
                        ...
                        deleteButton.onclick = () => deleteComments(idUser,item.route);
