...
<body>
    ...
    <script>
            ...
            return new Promise((resolve, reject) => {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        switch (typeValue) {
                            case "films":
                                nome = `${res.description}: Episode - ${res.properties.title}`;
                                link = `films?${res.uid}`;
                                break;
                            case "people":
        ...
        async function renderStarships(data) {
            ...
            const filmsLinks = await Promise.all(
                films.map(async (e) => {

                    let lastPart = e.split('/').slice(-2).join('/');

                    const resFilms = await fetchOuthers(lastPart);
                    console.log(resFilms)
                    return `
            <a href="${resFilms.link}" target="_self" class="btn btn-warning btn-sm">${resFilms.nome}</a>`;
                })
            );

            let filmsHTML = filmsLinks.join(" ");
                    ...
                    <div class="card-body">
                        <section class="details">
                            ...
                            <ul>
                                ...
                                <li><strong>Class:</strong> <span>${vehicle_class}</span></li>
